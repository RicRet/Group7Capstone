...
/** Explicit prop types exported so TS can find them when imported elsewhere */
export type HomepageProps = {
  onNavigate: (screen: string) => void;
  startNavigation: () => void;
};

export default function Homepage({ onNavigate, startNavigation }: HomepageProps) {
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: (bookmarkName?: string) => {
            if (
              bookmarkName &&
              bookmarkName.trim() &&
              !bookmarks.includes(bookmarkName.trim())
            ) {
  ...
  return (
    <View>
      ...
      {/* First Button */}
      <TouchableOpacity style={styles.button} onPress={() => onNavigate("addroute")}>
...
const styles = StyleSheet.create({
  // (kept your styles; tweak if needed)
  background: {
    flex: 1,
    width: "100%",
    height: "100%",
