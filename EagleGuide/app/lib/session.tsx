...
export type SessionUser = { id: string; roles: string[] };
  ...
  useEffect(() => {
    (async () => {
      ...
      if (t) {
        ...
        setAuthToken(t);
        try {
          const res = await http.get<{ userId: string; roles: string[] }>("/v1/users/me");
          setUser({ id: String(res.data.userId), roles: res.data.roles || [] });
        } catch {
          // token invalid; clear
